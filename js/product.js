...
function initShareButton() {
  const shareButton = document.getElementById('product-share-btn');
  ...
  shareButton.addEventListener('click', async () => {
    ...
    try {
      ...
      window.NW?.showToast('Link copied', 'success', 2000);
    } catch (error) {
      window.NW?.showToast('Unable to copy the link', 'error', 2000);
    }
  });
}
